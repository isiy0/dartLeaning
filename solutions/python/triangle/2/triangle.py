...
def equilateral(sides: list[float]) -> bool:
    divergent_sides = len(set(sides)) # Transforming the list of sides into a set that unifies repeated numbers and using the len() function
    ...
    return is_triangle(sides) and divergent_sides == 1


def isosceles(sides: list[float]) -> bool:
    divergent_sides = len(set(sides))
    return is_triangle(sides) and divergent_sides <= 2

def scalene(sides: list[float]) -> bool:
    divergent_sides = len(set(sides))
    return is_triangle(sides) and divergent_sides == 3
